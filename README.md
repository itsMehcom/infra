...
2. Copy `global.env.example` → `global.env` and fill in your domains (e.g., `DOMAIN`, `API_DOMAIN`).
3. Copy `postgres.env.example` → `postgres.env` and set your database superuser password.
4. See [`platform/postgres/README.md`](platform/postgres/README.md) for full database setup and onboarding.
5. Create backup dirs: `mkdir -p platform/postgres/backups/{base,wal,logical}`
